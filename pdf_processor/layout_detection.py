...
from pdf_processor.ocr import extract_table, extract_text
from utils.string import sanitize_and_join
...
def crop_and_save(image, detections, page_num, base_output_dir):
    ...
    for det in detections:
        ...
        if label == "Text":
            try:
                ...
                if text_result:
                    text_lines.append(f"{joined_text}")
            except Exception as e:
    ...
    if text_lines:
        print(f"[OCR] Saving text results to {text_output_path}")
